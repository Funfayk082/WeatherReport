...
class WeatherFragment(var cityName: String, var cities: MutableList<String>, var adapter: ViewAdapter) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        with(view) {
            var city = this.findViewById<TextView>(R.id.YourCityTV)
            var currTemp: String
            var currWC = 0
            ...
            weatherViewModel.getDate(getLocation(cityName)!!)
            weatherViewModel.weather.observe(
                ...
                weather?.let {
                    ...
                    et = 0
                    nt = 0
                    for (i in 0..5) mt += weather.hourly.temperature_2m[i].toInt()
                    ...
                    with(view) {
                        this.findViewById<TextView>(R.id.morningTemp).text =
                            (mt / 6).toString() + weather.current_units.temperature_2m
                        with(weather.daily) {
                        ...
                        this.findViewById<TextView>(R.id.dayTemp).text = (dt / 6).toString() + weather.current_units.temperature_2m
                        this.findViewById<TextView>(R.id.eveningTemp).text =
                            (et / 6).toString() + weather.current_units.temperature_2m
                        this.findViewById<TextView>(R.id.nightTemp).text = (nt / 6).toString() + weather.current_units.temperature_2m
                        this.findViewById<TextView>(R.id.tomorrowTempTV).text =
                            "${minTomorrowTemp}${weather.current_units.temperature_2m}...${maxTomorrowTemp}${weather.current_units.temperature_2m}"
                        this.findViewById<TextView>(R.id.afterTomorrowTempTV).text =
                            "${minAfterTomorrowTemp}${weather.current_units.temperature_2m}...${maxAfterTomorrowTemp}${weather.current_units.temperature_2m}"

                        this.findViewById<TextView>(R.id.afterAfterTomorrowTempStr).text = afterAfterTommorowString
                        this.findViewById<TextView>(R.id.afterAfterTomorrowTempTV).text =
                        ...
                        this.findViewById<TextView>(R.id.afterAfterAfterTomorrowTempStr).text = afterAfterAfterTomorrowStr
                        this.findViewById<TextView>(R.id.afterAfterAfterTomorrowTempTV).text =
                        ...
                        this.findViewById<TextView>(R.id.currentTemp).text = currTemp + weather.current_units.temperature_2m
                        ...
                        with(this.findViewById<ImageView>(R.id.weatherIcon)) {
                            when (weather.current.weather_code) {
                                0 -> setImageResource(R.drawable.sunny_svgrepo_com)
                                in 1..3 -> setImageResource(R.drawable.partly_cloudy_svgrepo_com)
                                in 45..48 -> setImageResource(R.drawable.fog_svgrepo_com)
                                in 51..67 -> setImageResource(R.drawable.rain_2_svgrepo_com)
                                in 71..86 -> setImageResource(R.drawable.snow_alt_1_svgrepo_com)
                            }
                        }
            ...
            this.findViewById<TextView>(R.id.YourCityTV).setOnClickListener {
                getCities()
            }
        }
    }
    ...
    fun getCities() {
        ...
        val dialogLayout = inflater.inflate(R.layout.cities, null)
        var adapter: ArrayAdapter<String> =
            ArrayAdapter<String>(this.requireContext(), android.R.layout.simple_list_item_1, cities)
        dialogLayout.findViewById<ListView>(R.id.cities).adapter = adapter

        with(builder) {
            setTitle("Выбранные города")
            setPositiveButton("Добавить") { _, _ ->
                getLocationWithCreate()
            }
        ...
        fun getLocationWithCreate() {
            ...
            with(builder) {
                ...
                setPositiveButton("Ок") { _, _ ->
                    ...
                    val geocoder = this@WeatherFragment.context?.let { Geocoder(it) }
                    ...
                    if (addresses.size > 0) {
                        ...
                        adapter.addFragment(WeatherFragment(editText.text.toString(), cities, adapter))
                        adapter.notifyDataSetChanged()
                    } else {
                        Log.e("Неверно!", "Города нет!!!")
                        Toast.makeText(null, "Город не найден!", Toast.LENGTH_LONG)
                        getLocationWithCreate()
                    }
                }
                setView(dialogLayout)
    ...
    fun getLocation(cityName: String): Address? {
        val geocoder = this@WeatherFragment.context?.let { Geocoder(it) }
